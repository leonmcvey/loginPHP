...
class Validator
{
    ...
    function field($name, $alias = null)
    {
        ...
        return $this;  // Returning an instance of the current class (Validator)
    }
    ...
    function alpha_space($ignore = [])
    {
        if ($this->next && $this->exists() && !preg_match('/^[a-zA-Z\s]+$/', $this->data[$this->current_field])) {
            $this->add_error_message('alpha_space');
            $this->next = false;
        }
        return $this;
    }
